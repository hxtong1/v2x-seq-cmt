...
def xyz_ego_transformation(xyz, l2g_r1, l2g_t1, l2g_r2, l2g_t2, pc_range, 
    ...
    if torch.__version__ < '1.9.2':
        g2l_r2 = torch.tensor(np.linalg.inv(l2g_r2.cpu().numpy())).type(torch.float).to(l2g_r2.device)
    else:
        g2l_r2 = torch.linalg.inv(l2g_r2).type(torch.float)
    # xyz = xyz @ l2g0[:3, :3].T + l2g0[:3, 3] - l2g1[:3, 3]
    # xyz = xyz @ g2l1[:3, :3].T
    xyz = xyz @ l2g_r1 + l2g_t1 - l2g_t2
    xyz = xyz @ g2l_r2
