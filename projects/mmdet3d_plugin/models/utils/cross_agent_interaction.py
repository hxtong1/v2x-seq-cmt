...
class CrossAgentSparseInteraction(nn.Module):
    ...
    def __init__(self, pc_range, inf_pc_range, embed_dims=256):
        ...
        self.cross_agent_align_pos = nn.Linear(
            self.embed_dims+9, self.embed_dims)
    ...
    def _loc_norm(self, locs, pc_range):
        ...
        locs[..., 0:1] = (locs[..., 0:1] - pc_range[0]) / \
            (pc_range[3] - pc_range[0])
        locs[..., 1:2] = (locs[..., 1:2] - pc_range[1]) / \
            (pc_range[4] - pc_range[1])
        locs[..., 2:3] = (locs[..., 2:3] - pc_range[2]) / \
    ...
    def _loc_denorm(self, ref_pts, pc_range):
        ...
        locs[:, 0:1] = (
            locs[:, 0:1] * (pc_range[3] - pc_range[0]) + pc_range[0])
        locs[:, 1:2] = (
            locs[:, 1:2] * (pc_range[4] - pc_range[1]) + pc_range[1])
        locs[:, 2:3] = (
            locs[:, 2:3] * (pc_range[5] - pc_range[2]) + pc_range[2])
    ...
    def _query_matching(self, inf_ref_pts, veh_ref_pts, veh_mask, veh_pred_dims):
        ...
        veh_ref_pts_expanded = veh_ref_pts.unsqueeze(
            1).expand(-1, inf_nums, -1)
        inf_ref_pts_expanded = inf_ref_pts.unsqueeze(
            0).expand(veh_nums, -1, -1)
        distances = torch.sqrt(
            torch.sum((veh_ref_pts_expanded - inf_ref_pts_expanded) ** 2, dim=-1))

        veh_pred_dims_expanded = veh_pred_dims.unsqueeze(
            1).expand(-1, inf_nums, -1).exp()
        diff = torch.abs(veh_ref_pts_expanded -
                         inf_ref_pts_expanded) / veh_pred_dims_expanded
        filter_mask = (diff[..., 0] <= 1) & (
            diff[..., 1] <= 1) & (diff[..., 2] <= 1)
    ...
    def _query_fusion(self, inf, veh, inf_idx, veh_idx, cost_matrix):
        ...
        matched_veh.query_feats = matched_veh.query_feats + \
            self.cross_agent_fusion(matched_inf.query_feats)
    ...
    def _query_complementation(self, inf, veh, inf_accept_idx, veh_accept_idx, fused):
        ...
        res_instances = Instances.cat(
            [res_instances, unmatched_veh[topk_indexes]])
    ...
    def forward(self, inf, veh, veh2inf_rt, threshold=0.3, debug=False, name='debug-vis.jpg'):
        ...
        if debug:
            self._vis(inf_ref_pts, veh_ref_pts, veh.scores,
                      vis_threshold=1.0, name="inf-"+name)

        # inf_ref_pts inf2veh
        calib_inf2veh = np.linalg.inv(veh2inf_rt.cpu().numpy().T)
        calib_inf2veh = inf_ref_pts.new_tensor(calib_inf2veh)
        inf_ref_pts = torch.cat((inf_ref_pts, torch.ones_like(
            inf_ref_pts[..., :1])), -1).unsqueeze(-1)
        inf_ref_pts = torch.matmul(
            calib_inf2veh, inf_ref_pts).squeeze(-1)[..., :3]
        ...
        if debug:
            self._vis(inf_ref_pts, veh_ref_pts, veh.scores,
                      vis_threshold=0.0, name=name)

        # matching
        veh_mask = torch.where(veh.scores >= 0.05)[0]
        veh_idx, inf_idx, cost_matrix = self._query_matching(
            inf_ref_pts, veh_ref_pts, veh_mask, veh.pred_boxes[..., [2, 3, 5]])  # veh.pred_boxes x,y,dx,dy,z,dz
        ...
        # inf.ref_pts = inf_ref_pts
        # cross-agent feature alignment
        inf2veh_r = calib_inf2veh[:3, :3].reshape(
            1, 9).repeat(inf.query_feats.shape[0], 1)
        inf.query_embeds = self.cross_agent_align_pos(
            torch.cat([inf.query_embeds, inf2veh_r], -1))
        inf.query_feats = self.cross_agent_align(
            torch.cat([inf.query_feats, inf2veh_r], -1))

        # cross-agent query fusion
        fused, veh_accept_idx, inf_accept_idx = self._query_fusion(
            inf, veh, inf_idx, veh_idx, cost_matrix)

        # cross-agent query complementation
        veh = self._query_complementation(
            inf, veh, inf_accept_idx, veh_accept_idx, fused)
        # pdb.set_trace()
    ...
    def forward_only_inf(self, inf, veh, veh2inf_rt, threshold=0.3, debug=False, name='debug-vis.jpg'):
        ...
        # cross-agent feature alignment
        inf2veh_r = calib_inf2veh[:3, :3].reshape(
            1, 9).repeat(inf.query.shape[0], 1)
