...
## UniV2X TODO: verify from_edge_line_token and to_edge_line_token since it corresponds to the lane direction
## UniV2X TODO: vefiry nuscenes line format, here we only consider two nodes
## UniV2X TODO: update left_lane_divider_segments and right_lane_divider_segments
## UniV2X TODO: verify the SPD map lane numbers - yizhuang9 with 18771 centerlines??
## UniV2X TODO: verify the node number in polygon_nodes - more than 500 nodes
def map_spd_to_nuscenes_lane(spd_map_data, nuscenes_map_data):
    ...
    for lane_id, lane in  tqdm(spd_map_data['LANE'].items()):
        ...
        # polygon_nodes = polygon_nodes - baidu_offset + dair_offset # align to spd calibration
...
## UniV2X TODO: check 'JUNCTION' in SPD vs 'road_segment' in nuScenes
...
## UniV2X TODO: check the drivable area, now we regard the union of {'lane', 'road_segment', 'ped_crossing'} as drivable area
...
def map_spd_to_nuscenes_canvas_edge(nuscenes_map_data):
    ...
    # nuscenes_map_data['canvas_edge'] = [nuscenes_canvas_edge_infos['max_x'],
    #                                                                                 nuscenes_canvas_edge_infos['max_y']]
...
def map_spd_to_nuscenes(maps_root, save_root):
    ...
    for location_name in location_names:
        ## UniV2X TODO: now we only consider 'yizhuang09' for example
...
def parse_args():
    ...
    parser.add_argument('--maps-root', type=str, default="/data/ad_sharing/datasets/v2x-seq/SPD/cooperative-vehicle-infrastructure/maps")
