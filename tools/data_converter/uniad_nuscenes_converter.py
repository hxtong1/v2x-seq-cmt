...
def create_nuscenes_infos(root_path,
    ...
    train_nusc_infos, val_nusc_infos = _fill_trainval_infos(
        nusc, nusc_can_bus, train_scenes, val_scenes, test, max_sweeps=max_sweeps,use_can_bus=use_can_bus,mono=mono)
...
def obtain_sensor2top(nusc,
    ...
    if nusc.dataroot in data_path:
        data_path = data_path.split(f'{nusc.dataroot}/')[-1]
    sweep = {
        'data_path': data_path,
        'type': sensor_type,
...
def export_2d_annotation(root_path, info_path, version, mono3d=True,mono=False):
    ...
    for info in mmcv.track_iter_progress(nusc_infos):
        for cam in camera_types:
            ...
            coco_infos = get_2d_boxes(
                ...
                mono3d=mono3d)
            (height, width, _) = mmcv.imread(osp.join(nusc.dataroot,cam_info['data_path'])).shape
